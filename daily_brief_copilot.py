#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
智能 Market Color 早报推送 (GitHub Copilot SDK版)
功能：抓取多源新闻 -> Copilot AI 总结 Market Color -> Bark 推送
"""

import os
import sys
import asyncio
import requests
from datetime import datetime

# 导入采集模块
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.append(BASE_DIR)

# 尝试导入
try:
    from copilot import CopilotClient
    from dotenv import load_dotenv
    import fetch_10jqka
    import fetch_WSJ
    import fetch_baidu
    import fetch_zhihu
    import fetch_weibo
    import fetch_jin10
    import fetch_wallstreetcn
    import fetch_36kr
    import fetch_bilibili
    import fetch_toutiao
    import fetch_github
    import fetch_juejin
except ImportError as e:
    print(f"导入模块失败: {e}")
    print("请确保已安装 requirements.txt 依赖，并安装 copilot-sdk：")
    print("cd ~/Desktop/VSCodePyScripts/TESTAI/copilot-sdk/python && pip install -e .")
    sys.exit(1)

# 加载环境变量 (优先读取当前目录的 .env)
load_dotenv(os.path.join(BASE_DIR, ".env"))

# 配置
BARK_KEY = os.getenv("BARK_KEY")

async def get_copilot_analysis(raw_news_text):
    """调用 GitHub Copilot SDK 生成 Market Color"""
    client = CopilotClient()
    await client.start()

    try:
        session = await client.create_session({
            "model": "gemini-3-flash",
            "system_message": {
                "content": "你是一个专业的全球宏观策略分析师。始终用中文回答，风格专业精炼。"
            }
        })
        
        prompt = f"""请根据以下抓取到的即时新闻，撰写一篇名为 'Market Color' 的简明市场评论。

要求：
1. 结构：【今日大势】、【核心逻辑】、【交易员备忘】。
2. 长度控制在 500 字以内。

新闻内容：
{raw_news_text}"""

        response = await session.send_and_wait({"prompt": prompt})
        
        if response and response.data:
            return response.data.content
        return "Copilot 返回内容为空"
    except Exception as e:
        return f"Copilot 分析失败: {e}"
    finally:
        try:
            await session.destroy()
        except:
            pass
        await client.stop()

def send_bark(title, content):
    """发送 Bark 通知"""
    if not BARK_KEY:
        print("⚠️  未设置 BARK_KEY，跳过 Bark 推送")
        return False

    url = "https://api.day.app/push"
    payload = {
        "body": content,
        "title": f"{title} [{datetime.now().strftime('%H:%M')}]",
        "group": "MarketBrief",
        "icon": "https://cdn-icons-png.flaticon.com/512/2503/2503903.png",
        "device_key": BARK_KEY,
        "level": "active"
    }
    try:
        resp = requests.post(url, json=payload, timeout=10)
        resp.raise_for_status()
        print("✅ 推送成功")
        return True
    except Exception as e:
        print(f"❌ 推送失败: {e}")
        return False

def main():
    print("🚀 正在启动智能早报采集 (Copilot)...")
    
    # 1. 抓取多源新闻
    raw_text_parts = []
    
    # 财经快讯类
    try:
        print("📡 抓取同花顺快讯...")
        ths = fetch_10jqka.get_live_news()
        for n in ths[:8]: 
            raw_text_parts.append(f"[同花顺] {n['title']}: {n.get('digest', '')}")
    except Exception as e:
        print(f"⚠️  同花顺抓取失败: {e}")
    
    try:
        print("📡 抓取华尔街日报...")
        wsj_live = fetch_WSJ.get_live_news()
        for n in wsj_live[:8]: 
            raw_text_parts.append(f"[华尔街日报] {n['title']}: {n.get('digest', '')}")
    except Exception as e:
        print(f"⚠️  华尔街日报抓取失败: {e}")
    
    try:
        print("📡 抓取金十数据...")
        jin10 = fetch_jin10.get_latest_articles()
        for n in jin10[:8]:
            summary = n.get('summary', '')
            raw_text_parts.append(f"[金十数据] {n['title']}: {summary}")
    except Exception as e:
        print(f"⚠️  金十数据抓取失败: {e}")
    
    try:
        print("📡 抓取华尔街见闻...")
        wallst = fetch_wallstreetcn.get_latest_articles()
        for n in wallst[:8]:
            raw_text_parts.append(f"[华尔街见闻] {n['title']}")
    except Exception as e:
        print(f"⚠️  华尔街见闻抓取失败: {e}")
    
    try:
        print("📡 抓取36氪快讯...")
        kr36 = fetch_36kr.get_latest_articles()
        for n in kr36[:5]:
            raw_text_parts.append(f"[36氪] {n['title']}")
    except Exception as e:
        print(f"⚠️  36氪抓取失败: {e}")
    
    # 热搜热榜类
    try:
        print("📡 抓取百度热搜...")
        baidu = fetch_baidu.get_latest_articles()
        for n in baidu[:5]:
            summary = n.get('summary', '')
            if summary:
                raw_text_parts.append(f"[百度热搜] {n['title']}: {summary[:60]}...")
            else:
                raw_text_parts.append(f"[百度热搜] {n['title']}")
    except Exception as e:
        print(f"⚠️  百度热搜抓取失败: {e}")
    
    try:
        print("📡 抓取知乎热榜...")
        zhihu = fetch_zhihu.get_latest_articles()
        for n in zhihu[:5]:
            raw_text_parts.append(f"[知乎] {n['title']}")
    except Exception as e:
        print(f"⚠️  知乎热榜抓取失败: {e}")
    
    try:
        print("📡 抓取今日头条...")
        toutiao = fetch_toutiao.get_latest_articles()
        for n in toutiao[:5]:
            raw_text_parts.append(f"[头条] {n['title']}")
    except Exception as e:
        print(f"⚠️  今日头条抓取失败: {e}")
    
    # 技术科技类
    try:
        print("📡 抓取GitHub Trending...")
        github = fetch_github.get_latest_articles()
        for n in github[:3]:
            raw_text_parts.append(f"[GitHub] {n['title']} ✰{n.get('stars', '')}")
    except Exception as e:
        print(f"⚠️  GitHub抓取失败: {e}")
    
    try:
        print("📡 抓取掘金热榜...")
        juejin = fetch_juejin.get_latest_articles()
        for n in juejin[:3]:
            raw_text_parts.append(f"[掘金] {n['title']}")
    except Exception as e:
        print(f"⚠️  掘金抓取失败: {e}")
    
    # 2. 汇总
    full_raw_text = "\n".join(raw_text_parts)
    
    if not full_raw_text.strip():
        print("❌ 未抓取到任何新闻")
        return
    
    # 3. 分析
    print("🧠 Copilot 正在分析市场脉络...")
    market_color = asyncio.run(get_copilot_analysis(full_raw_text))
    
    # 4. 打印并推送
    print("\n" + "="*30)
    print(market_color)
    print("="*30 + "\n")
    
    send_bark("智能 Market Color", market_color)

if __name__ == "__main__":
    main()
